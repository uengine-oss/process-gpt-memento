"""
Ingest 라우터 — 파싱 / 청킹 / 임베딩 관련 엔드포인트.

벡터 스토어에 콘텐츠를 적재하는 엔드포인트가 모두 여기 있다:
  - POST /process            (디스패처: local / drive / storage)
  - GET  /process/drive/status
  - POST /process/database
  - POST /process-output
  - POST /save-to-storage

검색(retrieve), 질의(query), 인증(auth), save-to-drive는 main.py에 남아 있다.
"""
...
from document_loader import DocumentProcessor, get_document_processor
...
from image_storage_utils import ImageStorageUtils, get_image_storage_utils
...
def _cleanup_drive_jobs() -> None:
    try:
        ...
        print(f"_cleanup_drive_jobs error: {e}")
...
# ---------------------------------------------------------------------------
# Per-storage processors
# ---------------------------------------------------------------------------

async def process_documents(request: ProcessRequest):
    """Process documents from the input directory (local)"""
